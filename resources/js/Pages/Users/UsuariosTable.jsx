...
const UsuariosTable = ({ users }) => {
    ...
    const handleDelete = async (user) => {
        if (window.confirm(`'Are you sure you want to delete this user ${user.name} ?'`)) {
            router.delete(route('home.destroy', user));
            setLoadedUsers(loadedUsers.filter(c => c.id !== user.id));
        }
    }
                    ...
                    {loadedUsers.map((user) => (
                        <tr key={user.id}>
                            <td className="px-6 py-4 whitespace-nowrap">
                                <span className="text-gray-900 font-bold inline-block text-center w-full">{user.id}</span>
                                <div className="flex flex-col items-center mt-1 space-y-1">
                                    <Link href={route('home.show', user)} className="bg-yellow-100 hover:bg-yellow-200 text-yellow-500 hover:text-yellow-900 rounded-2xl flex items-center p-2 gap-1"
                                    >
                                    ...
                                    <button className="bg-blue-50 hover:bg-blue-200 text-blue-500 hover:text-indigo-900 rounded-2xl flex items-center p-2"
                                        onClick={() => window.location.href = route('home.edit', user)}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-pencil-square" viewBox="0 0 16 16">
                                            <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456l-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                            <path fillRule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z" />
                                        </svg>
                                        <a href={route('home.edit', user)} className="text-indigo-600 hover:text-indigo-900 px-2">
                                            Edit
                                        </a>
                                    </button>
